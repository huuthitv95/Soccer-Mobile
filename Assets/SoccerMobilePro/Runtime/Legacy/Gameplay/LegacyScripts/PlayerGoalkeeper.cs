...
[UnityEngine.Scripting.APIUpdating.MovedFrom(true, sourceNamespace: "", sourceAssembly: "Assembly-CSharp", sourceClassName: "PlayerGolie")]
public class PlayerGoalkeeper : MonoBehaviour
{
	...
	void Update ()
	{
		if(ballScript.ownerPlayer == leftHand && Time.time - timeSinceCaught > 2)
		{
			...
			gameObject.GetComponent<PlayerGoalkeeper>().enabled = false;
			gameObject.GetComponent<PlayerGoalkeeperKickController>().enabled = true;
			return;
		}
