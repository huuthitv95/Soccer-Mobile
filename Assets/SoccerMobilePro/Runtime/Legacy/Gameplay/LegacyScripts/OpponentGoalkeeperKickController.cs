...
[UnityEngine.Scripting.APIUpdating.MovedFrom(true, sourceNamespace: "", sourceAssembly: "Assembly-CSharp", sourceClassName: "OpponentGolieKick")]
public class OpponentGoalkeeperKickController : MonoBehaviour
{
	[HideInInspector]
	...
	void Update ()
	{
		...
		else
		{
			...
			else if(GetComponent<Animation>()["saque_esquina"].enabled == true && GetComponent<Animation>()["saque_esquina"].normalizedTime >= 0.6f)
			{
				...
				gameObject.GetComponent<OpponentGoalkeeperKickController>().enabled = false;
				gameObject.GetComponent<OpponentGoalkeeper>().enabled = true;
			}
			else if(GetComponent<Animation>()["saque_esquina"].enabled == true && GetComponent<Animation>()["saque_esquina"].normalizedTime >= 0.5f)
			{
				if(ballKicked == false)
