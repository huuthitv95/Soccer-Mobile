...
| CLS-D03 | Dispute operations rehearsal | Competitive Ops | internal tournament | resolution time, audit completeness | [P1-04 append-only dispute/correction tests](p1-competition-and-integrity-foundation.md#evidence); operations export/SLA pending | SLA/reviewer separation pass | extend window/freeze bracket | `InValidation` |
...
| PCS-D01 | Rank/training economy test | Economy + Design | core-gated cohorts | cap clarity, power gap, regret | [P1-03 deterministic transaction evidence](p1-player-items-skills-and-progression-implementation.md) + economy simulation/playtest pending | no dominant paid path | disable recipe/compensate | `InValidation` |
| PCS-D02 | Skill taxonomy scenarios | Gameplay | beginner/expert; seeded squads | pick diversity, exploit rate | [P1-03 cap/eligibility evidence](p1-player-items-skills-and-progression-implementation.md) + gameplay scenario/balance report pending | situational value, bounded modifier | disable skill config | `InValidation` |
...
| CIE-D03 | Reconnect/forfeit network test | Match + Network | packet-loss/device matrix | resume rate, exploit rate, duration | [P1-04 reconnect/sequence tests](p1-competition-and-integrity-foundation.md#evidence); network/device traces pending | SLO/fairness pass | extended grace/unranked fallback | `InValidation` |
...
Trạng thái sau automated evidence P1-03: **25 `TestReady`, 8 `InValidation`, 12 `Blocked`, 0 `EvidenceReady`, 0 `Approved`**. Tám mục `InValidation` có automated evidence tại [nhật ký P0](p0-foundation-implementation.md), [nhật ký P1-01](p1-localization-settings-implementation.md), [nhật ký P1-02](p1-football-catalog-and-model-foundation.md) và [nhật ký P1-03](p1-player-items-skills-and-progression-implementation.md), nhưng vẫn thiếu human/device/fairness, economy simulation, backend/threat review hoặc owner gate tương ứng. `TestReady` chỉ xác nhận protocol đủ rõ; `Blocked` chỉ mở khi blocker nêu trong hàng đã được giải quyết.
