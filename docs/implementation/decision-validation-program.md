...
| PCS-D01 | Rank/training economy test | Economy + Design | core-gated cohorts | cap clarity, power gap, regret | simulation + playtest report | no dominant paid path | disable recipe/compensate | `TestReady` |
| PCS-D02 | Skill taxonomy scenarios | Gameplay | beginner/expert; seeded squads | pick diversity, exploit rate | scenario traces + balance report | situational value, bounded modifier | disable skill config | `TestReady` |
